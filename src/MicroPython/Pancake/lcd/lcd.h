/*
 * LCD Driver for the Marauder Pancake (ESP32-C5, ST7796 320x480 SPI).
 *
 * Structured after the Cardputer driver: an 8-bit RGB332 framebuffer is drawn
 * into by the shape/text routines and expanded through a palette to RGB565 when
 * it is pushed out over SPI. At 320x480 the framebuffer is 150 KB, which does
 * not fit in the C5's internal SRAM, so it lives in PSRAM.
 */
...
#define LCD_HEIGHT 480   // Vertical resolution
#define BITS_PER_PIXEL 8 // Number of image display bits of the display screen

#define LCD_BACKLIGHT_PIN 26       // LCD Backlight Pin
#define LCD_BACKLIGHT_PWM_HZ 20000 // LCD Backlight PWM Frequency

#define LCD_DEFAULT_BRIGHTNESS 100 // Default brightness (0-100)
...
extern "C"
{
    ...
    void lcd_fill_triangle_alpha(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color, uint8_t alpha); // Draw a filled triangle blended over what is already there
