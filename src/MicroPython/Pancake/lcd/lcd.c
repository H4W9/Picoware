...
// The ESP-IDF ships no ST7796 vendor driver, but the ST7796 speaks the same
// generic command set as the ST7789 for everything esp_lcd itself drives
// (SWRESET/SLPOUT/COLMOD/MADCTL/CASET/RASET/RAMWR). Only the power and gamma
// tuning below is ST7796-specific, so the ST7789 driver is reused and these
// registers are written afterwards. Values match the ST7796 init that this
// panel is already known to work with under TFT_eSPI.
static esp_err_t lcd_send_st7796_tuning(void)
{
    ...
    // 0xF0 is the command-set unlock; the tuning registers are inaccessible
    // until it is sent, and must be locked again afterwards.
...
static esp_err_t display_setup_panel(void)
{
    ...
    // MISO belongs to the SD card, not the panel, but whichever driver wins the
    // race to initialize this bus fixes its pin map for everyone. Claim MISO
    // here so the SD card still works when the display comes up first.
    ...
    // ESP_ERR_INVALID_STATE means the SD card already brought the bus up; in
    // that case it owns the bus and must be the one to free it.
...
static esp_err_t lcd_alloc_framebuffer(void)
{
    ...
    // 320 * 480 = 150 KB. This only fits in PSRAM on the C5.
...
static esp_err_t display_init(void)
{
    ...
    // Raise the backlight only once a black frame is on the panel, so boot
    // doesn't flash whatever noise the RAM powered up with.
